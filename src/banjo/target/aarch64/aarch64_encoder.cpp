...
void AArch64Encoder::encode_b(mcode::Instruction &instr) {
    ...
    text.add_symbol_use(m_target.get_basic_block().label, BinSymbolUseKind::LABEL_BRANCH);
...
std::uint32_t AArch64Encoder::encode_addr(Address &addr, unsigned imm_shift) {
    ...
    if (addr.mode == AddressingMode::OFFSET_CONST) {
        ...
        std::uint32_t r_off = encode_gp_reg(addr.offset_reg.reg.get_physical_reg());
        unsigned shift = addr.offset_reg.shift ? 1 : 0;
        return 0x206800 | (r_off << 16) | (shift << 12) | (r_base << 5);
