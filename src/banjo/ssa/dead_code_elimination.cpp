...
void DeadCodeElimination::remove_unused_instrs(Function &func) {
    std::unordered_map<ssa::VirtualRegister, unsigned> reg_uses;

    for (BasicBlock &block : func) {
        for (Instruction &instr : block) {
            passes::PassUtils::iter_regs(instr.get_operands(), [&reg_uses](ssa::VirtualRegister reg) {
                reg_uses[reg] += 1;
            });
        }
    }
    ...
    while (changed) {
        ...
        for (BasicBlock &block : func) {
            ...
            for (InstrIter iter = instrs.get_last_iter(); iter != instrs.get_first_iter().get_prev(); --iter) {
                if (iter->get_opcode() == ssa::Opcode::CALL) {
                    continue;
                }
                ...
                if (iter->get_dest() && reg_uses[*iter->get_dest()] == 0) {
                    ...
                    passes::PassUtils::iter_regs(iter->get_operands(), [&reg_uses](ssa::VirtualRegister reg) {
                        reg_uses[reg] -= 1;
                    });
                    block.remove(iter);
