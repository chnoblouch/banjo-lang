...
protected:
    void emit_data(const void *data, std::size_t size) { stream.write(reinterpret_cast<const char *>(data), size); }
    void emit_data(const WriteBuffer &buffer) { emit_data(buffer.get_data().data(), buffer.get_size()); }
