...
void Writer::write(Module &mod) {
    if (!mod.get_structures().empty()) {
        for (ir::Structure *struct_ : mod.get_structures()) {
...
void Writer::write_basic_block(BasicBlock &basic_block) {
    ...
    for (ir::Instruction &instr : basic_block) {
        ...
        if (instr.get_flags() & ir::Instruction::FLAG_ARG_STORE) stream << " !arg_store";
        if (instr.get_flags() & ir::Instruction::FLAG_SAVE_ARG) stream << " !save_arg";
