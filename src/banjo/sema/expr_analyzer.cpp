...
ExprAnalyzer::ExprAnalyzer(SemanticAnalyzer &analyzer, bool eval_meta_exprs /* = true */)
  : analyzer(analyzer),
    eval_meta_exprs(eval_meta_exprs) {}
...
Result ExprAnalyzer::analyze_uncoerced(sir::Expr &expr) {
    ...
    if (auto reference_type = expr.get_type().match<sir::ReferenceType>()) {
        expr = analyzer.create(sir::UnaryExpr{
...
Result ExprAnalyzer::analyze_array_literal(sir::ArrayLiteral &array_literal, sir::Expr &out_expr) {
    ...
    if (array_literal.values.size() == 1 && array_literal.values[0].is_type()) {
        out_expr = analyzer.create(sir::ArrayType{
        ...
        array_literal.type = analyzer.create(sir::PseudoType{
            .ast_node = nullptr,
...
Result ExprAnalyzer::analyze_string_literal(sir::StringLiteral &string_literal) {
    string_literal.type = analyzer.create(sir::PseudoType{
        .ast_node = nullptr,
...
Result ExprAnalyzer::analyze_struct_literal(sir::StructLiteral &struct_literal) {
    ...
    if (struct_literal.type) {
        result = ExprAnalyzer(analyzer).analyze_type(struct_literal.type);
        if (result != Result::SUCCESS) {
            return result;
        }
    }
    ...
    for (sir::StructLiteralEntry &entry : struct_literal.entries) {
        ...
        result = ExprAnalyzer(analyzer).analyze_value_uncoerced(entry.value);
        if (result != Result::SUCCESS) {
            return result;
        }
    }
...
Result ExprAnalyzer::analyze_map_literal(sir::MapLiteral &map_literal, sir::Expr &out_expr) {
    ...
    if (first_key_category == sir::ExprCategory::VALUE && first_value_category == sir::ExprCategory::VALUE) {
        map_literal.type = analyzer.create(sir::PseudoType{
            .ast_node = nullptr,
        ...
        out_expr = analyzer.create(sir::MapType{
...
Result ExprAnalyzer::analyze_closure_literal(sir::ClosureLiteral &closure_literal, sir::Expr &out_expr) {
    ...
    sir::Param data_ptr_param{
        .ast_node = nullptr,
        .name = {},
        .type = analyzer.create(sir::PrimitiveType{
            .ast_node = nullptr,
    ...
    sir::TupleExpr *data = analyzer.create(sir::TupleExpr{
        .ast_node = nullptr,
        .type = data_type,
    ...
    sir::Expr callee = analyzer.create(sir::SymbolExpr{
        .ast_node = nullptr,
        .type = &new_def.type,
        .symbol = &new_def,
    });
    ...
    sir::Expr func_ptr = analyzer.create(sir::CastExpr{
        .ast_node = nullptr,
        .type = addr_type,
...
Result ExprAnalyzer::analyze_binary_expr(sir::BinaryExpr &binary_expr, sir::Expr &out_expr) {
    ...
    if (Utils::is_one_of(binary_expr.op, {sir::BinaryOp::EQ, sir::BinaryOp::NE})) {
        lhs_result = ExprAnalyzer(analyzer).analyze_uncoerced(binary_expr.lhs);
        rhs_result = ExprAnalyzer(analyzer).analyze_uncoerced(binary_expr.rhs);
    } else {
        lhs_result = ExprAnalyzer(analyzer).analyze_value_uncoerced(binary_expr.lhs);
        rhs_result = ExprAnalyzer(analyzer).analyze_value_uncoerced(binary_expr.rhs);
    }
...
Result ExprAnalyzer::analyze_unary_expr(sir::UnaryExpr &unary_expr, sir::Expr &out_expr) {
    ...
    if (unary_expr.op == sir::UnaryOp::REF) {
        ...
        unary_expr.type = analyzer.create(sir::PointerType{
            .ast_node = nullptr,
...
Result ExprAnalyzer::analyze_cast_expr(sir::CastExpr &cast_expr) {
    ...
    partial_result = ExprAnalyzer(analyzer).analyze_value(cast_expr.value);
    if (partial_result != Result::SUCCESS) {
        result = partial_result;
    }

    partial_result = ExprAnalyzer(analyzer).analyze_type(cast_expr.type);
    if (partial_result != Result::SUCCESS) {
        result = partial_result;
    }
...
Result ExprAnalyzer::analyze_call_expr(sir::CallExpr &call_expr, sir::Expr &out_expr) {
    ...
    if (auto dot_expr = call_expr.callee.match<sir::DotExpr>()) {
        ...
        partial_result = ExprAnalyzer(analyzer).analyze_value(call_expr.callee);
    }
    ...
    for (unsigned i = first_arg_to_analyze; i < call_expr.args.size(); i++) {
        ...
        partial_result = ExprAnalyzer(analyzer).analyze_value_uncoerced(arg);
        if (partial_result != Result::SUCCESS) {
            result = Result::ERROR;
        }
    }
...
Result ExprAnalyzer::analyze_dot_expr_callee(sir::DotExpr &dot_expr, sir::CallExpr &out_call_expr, bool &is_method) {
    ...
    if (auto struct_def = lhs_type.match_symbol<sir::StructDef>()) {
        ...
        if (field) {
            ...
            out_call_expr.callee = analyzer.create(sir::FieldExpr{
...
Result ExprAnalyzer::analyze_union_case_literal(sir::CallExpr &call_expr, sir::Expr &out_expr) {
    ...
    out_expr = analyzer.create(sir::UnionCaseLiteral{
        .ast_node = nullptr,
...
Result ExprAnalyzer::analyze_static_array_type(sir::StaticArrayType &static_array_type) {
    ...
    partial_result = ExprAnalyzer(analyzer).analyze_type(static_array_type.base_type);
    if (partial_result != Result::SUCCESS) {
        result = Result::ERROR;
    }

    partial_result = ExprAnalyzer(analyzer).analyze_value(static_array_type.length);
    if (partial_result != Result::SUCCESS) {
        return Result::ERROR;
    }
...
Result ExprAnalyzer::analyze_optional_type(sir::OptionalType &optional_type, sir::Expr &out_expr) {
    ...
    partial_result = ExprAnalyzer(analyzer).analyze_type(optional_type.base_type);
    if (partial_result != Result::SUCCESS) {
        return Result::ERROR;
    }
...
Result ExprAnalyzer::analyze_result_type(sir::ResultType &result_type, sir::Expr &out_expr) {
    ...
    partial_result = ExprAnalyzer(analyzer).analyze_type(result_type.value_type);
    if (partial_result != Result::SUCCESS) {
        result = Result::ERROR;
    }

    partial_result = ExprAnalyzer(analyzer).analyze_type(result_type.error_type);
    if (partial_result != Result::SUCCESS) {
        result = Result::ERROR;
    }
...
Result ExprAnalyzer::analyze_array_type(sir::ArrayType &array_type, sir::Expr &out_expr) {
    ...
    partial_result = ExprAnalyzer(analyzer).analyze_type(array_type.base_type);
    if (partial_result != Result::SUCCESS) {
        return Result::ERROR;
    }
...
Result ExprAnalyzer::analyze_map_type(sir::MapType &map_type, sir::Expr &out_expr) {
    ...
    partial_result = ExprAnalyzer(analyzer).analyze_type(map_type.key_type);
    if (partial_result != Result::SUCCESS) {
        result = Result::ERROR;
    }

    partial_result = ExprAnalyzer(analyzer).analyze_type(map_type.value_type);
    if (partial_result != Result::SUCCESS) {
        result = Result::ERROR;
    }
...
Result ExprAnalyzer::analyze_range_expr(sir::RangeExpr &range_expr) {
    ...
    lhs_result = ExprAnalyzer(analyzer).analyze_uncoerced(range_expr.lhs);
    rhs_result = ExprAnalyzer(analyzer).analyze_uncoerced(range_expr.rhs);
...
void ExprAnalyzer::analyze_tuple_expr(sir::TupleExpr &tuple_expr) {
    ...
    tuple_expr.type = analyzer.create(sir::TupleExpr{
        .ast_node = nullptr,
        .type = nullptr,
        .exprs = types,
    });
...
Result ExprAnalyzer::analyze_star_expr(sir::StarExpr &star_expr, sir::Expr &out_expr) {
    ...
    if (star_expr.value.is_type()) {
        out_expr = analyzer.create(sir::PointerType{
...
Result ExprAnalyzer::analyze_meta_field_expr(sir::MetaFieldExpr &meta_field_expr) {
    ...
    if (meta_field_expr.field.value == "size") {
        meta_field_expr.type = sir::create_primitive_type(*analyzer.cur_sir_mod, sir::Primitive::USIZE);
    } else if (Utils::is_one_of(meta_field_expr.field.value, {"is_pointer", "is_struct", "is_enum", "is_resource"})) {
        meta_field_expr.type = sir::create_primitive_type(*analyzer.cur_sir_mod, sir::Primitive::BOOL);
    }
...
Result ExprAnalyzer::analyze_meta_call_expr(sir::MetaCallExpr &meta_call_expr) {
    ...
    if (auto meta_field_expr = meta_call_expr.callee.match<sir::MetaFieldExpr>()) {
        if (meta_field_expr->field.value == "has_method") {
            meta_call_expr.type = sir::create_primitive_type(*analyzer.cur_sir_mod, sir::Primitive::BOOL);
        }
    }
    ...
    if (decl_block) {
        ...
        if (!lookup_result.symbol) {
            ...
            if (iter != analyzer.incomplete_decl_blocks.end()) {
                ...
                lookup_result = analyzer.symbol_ctx.look_up_rhs_local(dot_expr);
            }
        }
...
Result ExprAnalyzer::analyze_operator_overload_call(
    ...
    sir::CallExpr *call_expr = analyzer.create(sir::CallExpr{
...
Result ExprAnalyzer::specialize(sir::FuncDef &func_def, std::span<sir::Expr> generic_args, sir::Expr &inout_expr) {
    ...
    inout_expr = analyzer.create(sir::SymbolExpr{
        .ast_node = inout_expr.get_ast_node(),
...
sir::Expr ExprAnalyzer::create_isize_cast(sir::Expr value) {
    ...
    return analyzer.create(sir::CastExpr{
