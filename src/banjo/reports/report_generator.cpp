...
void ReportGenerator::report_err_cyclical_definition(ASTNode *ast_node) {
    report_error("cyclical definition", ast_node);
}
...
void ReportGenerator::report_err_missing_generic_args(sir::Expr expr, sir::Symbol symbol) {
    std::string_view kind = symbol_kind_name(symbol);
    sir::Ident &ident = symbol.get_ident();
    std::span<sir::GenericParam *> params = symbol.get_generic_params();
...
void ReportGenerator::report_err_unexpected_generic_arg_count(sir::BracketExpr &bracket_expr, sir::Symbol symbol) {
    std::string_view kind = symbol_kind_name(symbol);
    sir::Ident &ident = symbol.get_ident();
    std::span<sir::GenericParam *> params = symbol.get_generic_params();
